...
def get_histogram(img_patches, bin):
    ...
    for i in range(8):
        ...
        hist = np.zeros(bin)
        angle_idx = np.int32(bin * angle/360)
        for i in range (bin):
            hist[i] = np.sum(mag[angle_idx == i])
...
def draw_match(result):
    ...
    for i in range(4):
        match_img = cv2.line(match_img, (points[i][0], points[i][1]), (points[result[i]][0]+600, points[result[i]][1]), (0,0,0), 3)
